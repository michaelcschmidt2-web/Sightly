...
test('app keeps guest mode local and attempts Supabase email auth only for email entry', () => {
  ...
  assert.match(app, /subscribeToAuthChanges/)
  assert.match(app, /beginGuestOnboarding/)
  assert.match(app, /authMode: 'guest'/)
  assert.match(app, /authMode: 'email'/)
  assert.match(app, /saveCloudSnapshot\(\{ userId, check \}\)/)
  assert.match(app, /saveCloudFeedback\(\{[\s\S]*userId,[\s\S]*guestId: feedbackGuestId,/)
